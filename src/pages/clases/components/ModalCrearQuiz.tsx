...
const ModalCrearQuiz = ({
  ...
  const [form, setForm] = useState({
    ...
    sin_limite:       quiz?.tiempo_limite === null || quiz?.tiempo_limite === undefined,
  });
  ...
  const handleGuardar = async () => {
    ...
    const tiempo_limite = form.sin_limite ? null : Number(form.tiempo_limite) || null;

    let resultado;
    if (esEdicion) {
      resultado = await dispatch(editarQuiz({
        quiz_id:           quiz._id,
        titulo:            form.titulo,
      ...
      resultado = await dispatch(crearQuiz({
        contexto, tema_id, ayudantia_id, capitulo_id, curso_id,
        titulo:            form.titulo,
        descripcion:       form.descripcion,
        tiempo_limite,
  ...
  return (
    <Dialog open onClose={onClose} maxWidth="sm" fullWidth
      ...
      <DialogTitle sx={{ bgcolor: "#2d5be3", color: "white", display: "flex", alignItems: "center", gap: 1.5, py: 2 }}>
        <QuizIcon />
      ...
      <DialogContent sx={{ pt: 3, pb: 1 }}>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3">
            <TextField
              label="Tiempo límite (minutos)" type="number"
              value={form.sin_limite ? "" : form.tiempo_limite}
              onChange={(e) => setForm((f) => ({ ...f, tiempo_limite: e.target.value }))}
              size="small" disabled={form.sin_limite}
              sx={{ flex: 1, "& .MuiOutlinedInput-root": { borderRadius: 2 } }}
            />
            <FormControlLabel
              control={
                <Switch size="small" checked={form.sin_limite}
                  onChange={(e) => setForm((f) => ({ ...f, sin_limite: e.target.checked }))}
                  sx={{ "& .MuiSwitch-thumb": { bgcolor: form.sin_limite ? "#2d5be3" : "#ccc" } }} />
              }
              label={<Typography variant="caption" sx={{ color: "#6793ba" }}>Sin límite</Typography>}
            />
          </div>
          <div className="flex gap-3">
            <TextField
              label="Intentos (0=ilimitados)" type="number"
              value={form.intentos}
      ...
      <DialogActions sx={{ px: 3, py: 2, justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Button onClick={handleGuardar} variant="contained"
            ...
            sx={{ bgcolor: "#2d5be3", borderRadius: 2, px: 3, fontWeight: 600, boxShadow: "none",
              "&:hover": { bgcolor: "#1a3cb0", boxShadow: "none" },
              "&:disabled": { bgcolor: "#b0c0f5" } }}>
            {guardando ? "Guardando..." : esEdicion ? "Actualizar" : "Crear quiz"}
          </Button>
        </div>
      </DialogActions>
    </Dialog>
