...
const Ayudantias = () => {
  ...
  const handleDesplegarPagina = async () => {
    ...
    setMsgDeploy(null);
    try {
      const resp = await fetchConToken(`api/admin/publicar-pagina/ayudantias/${capitulo_id}`, {}, "POST");
    ...
    <div className="min-h-screen bg-[#f0f4f8] p-6">
      ...
      {!isLoading && ayudantias.length === 0 && !error && (
        <div className="flex flex-col items-center gap-3 py-20 animate-fadeIn">
          ...
          <Typography variant="body1" sx={{ color: "#6793ba", fontWeight: 500 }}>No hay ayudantías</Typography>
          <Typography variant="body2" sx={{ color: "#8daecb" }}>Crea la primera con el botón "Nueva ayudantía"</Typography>
        </div>
      )}
      ...
      {!isLoading && ayudantias.length > 0 && (
        <div className="flex flex-col gap-4 animate-fadeIn">
          {ayudantias.map((ay, idx) => (
            <AyudantiaCard key={ay._id} ayudantia={ay} curso_id={curso_id!}
              capitulo_id={capitulo_id!} esPrimero={idx === 0} esUltimo={idx === ayudantias.length - 1} />
          ))}
        </div>
      )}
    </div>
