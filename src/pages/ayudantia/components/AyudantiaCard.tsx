...
import {
  editarAyudantia, eliminarAyudantia,
...
const iconBtnSx = {
  ...
  border: "0.5px solid #E2E8F0", bgcolor: "#F8FAFC", color: "#94A3B8",
...
const ModalEliminarAyudantia = ({
  ayudantia, onClose,
}: { ayudantia: IAyudantia; onClose: () => void }) => {
  ...
  return (
    <Dialog open onClose={onClose} maxWidth="xs" fullWidth
      ...
      <DialogTitle sx={{
        ...
        py: 2, px: 3, fontFamily: "Georgia, serif", fontSize: "17px", fontWeight: "normal",
      }}>
        <div style={{ width: 30, height: 30, borderRadius: 8, background: "#DC2626", border: "1px solid #EF4444", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      <DialogContent sx={{ pt: 3, pb: 1 }}>
        <Typography variant="body2" sx={{ color: "#334155", mb: 1.5 }}>
          ¿Eliminar <strong style={{ color: "#0F172A" }}>{ayudantia.nombre}</strong>?
        </Typography>
      ...
      <DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
        ...
        <Button onClick={handleEliminar} variant="contained" disabled={eliminando}
          ...
          sx={{ bgcolor: "#DC2626", borderRadius: "8px", px: 2.5, fontWeight: 500, fontSize: "13px", textTransform: "none", boxShadow: "none", "&:hover": { bgcolor: "#B91C1C", boxShadow: "none" } }}>
...
const ModalEnunciado = ({
  ...
  const handleGuardar = async () => {
    setGuardando(true);
    await dispatch(editarAyudantia({ ayudantia_id: ayudantia._id, enunciado: contenido }));
    setGuardando(false);
    onClose();
  };
...
const RecursoSlot = ({
  exists, label, icon, iconColor,
  onVer, onEditar, onEliminar, onCrear,
}: RecursoSlotProps) => {
    ...
    return (
      <button
        ...
        style={{
          ...
          padding: "0 10px", height: 26, borderRadius: 20,
          border: "0.5px dashed #CBD5E1", background: "transparent",
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", borderRadius: 20, border: "0.5px solid #E2E8F0", overflow: "hidden", height: 26 }}>
        <button
          onClick={onVer}
          style={{
            background: "transparent", border: "none",
            borderRight: "0.5px solid #E2E8F0", padding: "0 10px", height: "100%",
        ...
        >
          <span style={{ color: iconColor, fontSize: 13, display: "flex", alignItems: "center" }}>{icon}</span>
          {label}
        </button>
        <button
          onClick={(e) => setMenuAnchor(e.currentTarget)}
          style={{
            background: "transparent", border: "none", padding: "0 7px", height: "100%",
            cursor: "pointer", display: "flex", alignItems: "center", color: "#94A3B8", fontFamily: "inherit",
          }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#F8FAFC"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}
        >
      ...
      <Menu
        ...
        slotProps={{ paper: { sx: { mt: 0.5, minWidth: 140, borderRadius: "8px", border: "0.5px solid #E2E8F0", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" } } }}
      >
        <MenuItem onClick={() => { setMenuAnchor(null); onVer?.(); }} sx={{ gap: 1.5, py: 1, fontSize: 13, "&:hover": { bgcolor: "#F8FAFC" } }}>
          <ListItemIcon><VisibilityIcon sx={{ fontSize: 14, color: "#2563EB" }} /></ListItemIcon>
          <Typography variant="body2" sx={{ color: "#334155" }}>Ver</Typography>
        </MenuItem>
        <MenuItem onClick={() => { setMenuAnchor(null); onEditar?.(); }} sx={{ gap: 1.5, py: 1, fontSize: 13, "&:hover": { bgcolor: "#F8FAFC" } }}>
          <ListItemIcon><EditIcon sx={{ fontSize: 14, color: "#2563EB" }} /></ListItemIcon>
          <Typography variant="body2" sx={{ color: "#334155" }}>Editar</Typography>
        </MenuItem>
        <Divider sx={{ borderColor: "#F1F5F9", my: 0.5 }} />
        <MenuItem onClick={() => { setMenuAnchor(null); onEliminar?.(); }} sx={{ gap: 1.5, py: 1, fontSize: 13, "&:hover": { bgcolor: "#FFF5F5" } }}>
          <ListItemIcon><DeleteIcon sx={{ fontSize: 14, color: "#EF4444" }} /></ListItemIcon>
...
const AyudantiaCard = ({
  ayudantia, curso_id, capitulo_id,
  ...
  const quiz     = quizzesSafe.find((q) => q.ayudantia_id === ayudantia._id && q.contexto === "ayudantia") as IQuiz | undefined;
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Escape") { setNombre(ayudantia.nombre); setEditandoNombre(false); }
  };
  ...
  const syncCount  = ayudantia.canvas_deployments.filter((d) => d.status === "synced").length;
  const hasContent = !!ayudantia.enunciado || !!solucion || !!video || !!quiz;
  ...
  return (
    <>
      <Card
        elevation={0}
        sx={{
          borderRadius: "12px", border: "0.5px solid #E2E8F0", bgcolor: "white",
          overflow: "hidden", transition: "box-shadow 0.15s",
          boxShadow: isDragging ? "0 8px 24px rgba(0,0,0,0.12)" : "0 1px 3px rgba(0,0,0,0.04)",
          "&:hover": { boxShadow: isDragging ? "0 8px 24px rgba(0,0,0,0.12)" : "0 4px 16px rgba(0,0,0,0.07)" },
        }}
      >
        <CardContent sx={{ p: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px" }}>
            ...
            <div
              {...dragHandleProps}
              style={{ color: "#CBD5E1", cursor: "grab", flexShrink: 0, display: "flex", alignItems: "center", touchAction: "none" }}
            ...
            <div style={{
              ...
              color: hasContent ? "#1E3A8A" : "#94A3B8", flexShrink: 0,
            }}>
              {ayudantia.position}
            </div>
              ...
              {editandoNombre ? (
                <div className="flex items-center gap-1">
                  ...
                  <Tooltip title="Guardar">
                    <span>
                      <IconButton size="small" onClick={handleGuardarNombre} disabled={guardandoNombre} sx={{ color: "#2563EB" }}>
                        {guardandoNombre ? <CircularProgress size={14} sx={{ color: "#2563EB" }} /> : <CheckIcon sx={{ fontSize: 16 }} />}
                      </IconButton>
                    </span>
                  </Tooltip>
                  <Tooltip title="Cancelar">
                    <IconButton size="small" onClick={() => { setNombre(ayudantia.nombre); setEditandoNombre(false); }} sx={{ color: "#94A3B8" }}>
                ...
                <>
                  <Typography variant="body2" sx={{ fontWeight: 500, color: "#0F172A", fontSize: "13.5px", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", mb: 0.3 }}>
                    {ayudantia.nombre}
                  </Typography>
            ...
            {!editandoNombre && (
              <div style={{ display: "flex", gap: 4, alignItems: "center", flexShrink: 0 }}>
                <Tooltip title={`Canvas: ${ayudantia.published_canvas ? "publicado" : "oculto"}`}>
                  <span>
                    {togglingCanvas
                      ? <CircularProgress size={14} sx={{ color: "#2563EB", mx: 0.5 }} />
                      : <IconButton size="small" onClick={handleToggleCanvas} sx={ayudantia.published_canvas ? iconBtnActiveSx : iconBtnSx}>
                          <SchoolIcon sx={{ fontSize: 14 }} />
                        </IconButton>}
                  </span>
                </Tooltip>
                <Tooltip title={`Plataforma: ${ayudantia.published_api ? "publicado" : "oculto"}`}>
                  <span>
                    {togglingApi
                      ? <CircularProgress size={14} sx={{ color: "#2563EB", mx: 0.5 }} />
                      : <IconButton size="small" onClick={handleToggleApi} sx={ayudantia.published_api ? iconBtnActiveSx : iconBtnSx}>
                          {ayudantia.published_api ? <PublicIcon sx={{ fontSize: 14 }} /> : <PublicOffIcon sx={{ fontSize: 14 }} />}
                        </IconButton>}
                  </span>
                </Tooltip>
                ...
                >
                  <MenuItem onClick={() => { setMenuAnchor(null); setEditandoNombre(true); }} sx={{ gap: 1.5, py: 1, "&:hover": { bgcolor: "#F8FAFC" } }}>
                    <ListItemIcon><EditOutlinedIcon sx={{ fontSize: 15, color: "#2563EB" }} /></ListItemIcon>
                    <Typography variant="body2" sx={{ color: "#334155" }}>Editar nombre</Typography>
                  </MenuItem>
                  <Divider sx={{ borderColor: "#F1F5F9", my: 0.5 }} />
          ...
          {/* ── Enunciado preview ── */}
          <div style={{ borderTop: "0.5px solid #F1F5F9", padding: "10px 14px", background: "#FAFAFA" }}>
            {ayudantia.enunciado ? (
              <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.6, marginBottom: 6, maxHeight: 60, overflow: "hidden" }}>
                <TiptapRenderer>{ayudantia.enunciado}</TiptapRenderer>
              </div>
            ) : (
              <p style={{ fontSize: 12, color: "#CBD5E1", marginBottom: 6 }}>Sin enunciado</p>
            )}
            <button
          ...
          {/* ── Recursos ── */}
          <div style={{ borderTop: "1px solid #E2E8F0", padding: "8px 14px", display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <RecursoSlot
              exists={!!solucion}
              label="Solución"
              ...
              onEliminar={() => solucion && dispatch(eliminarSolucionTexto({ solucion_id: solucion._id }))}
            ...
            <RecursoSlot
              ...
              onEliminar={() => video && dispatch(eliminarVideo({ video_id: video._id }))}
            ...
            <RecursoSlot
              ...
              iconColor="#2563EB"
              onVer={() => quiz && navigate(`/cursos/${curso_id}/capitulos/${capitulo_id}/ayudantias/${ayudantia._id}/quiz/${quiz._id}`)}
              onEditar={() => setModalQuiz(true)}
              onEliminar={() => quiz && dispatch(eliminarQuiz({ quiz_id: quiz._id }))}
      ...
      {/* ── Modales ── */}
      {modalEliminar  && <ModalEliminarAyudantia ayudantia={ayudantia} onClose={() => setModalEliminar(false)} />}
