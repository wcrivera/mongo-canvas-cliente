...
const Header = ({ curso_id, capitulo, setMsgDeploy }: HeaderProps) => {
  ...
  const { ayudantias } = useAppSelector((s) => s.ayudantiaMongo);

  const [desplegando, setDesplegando] = useState(false);

  const handleTabNav = (key: string) => {
    if (key === "ayudantias") return;
    if (key === "clases")     navigate(`/cursos/${curso_id}/capitulos/${capitulo._id}/clases`);
    if (key === "ejercicios") navigate(`/cursos/${curso_id}/capitulos/${capitulo._id}/ejercicios`);
  };
  ...
  const handleDesplegarPagina = async () => {
    ...
    try {
      const resp = await fetchConToken(
        `api/admin/publicar-pagina/ayudantias/${capitulo._id}`, {}, "POST"
      );
