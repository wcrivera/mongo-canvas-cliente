...
const PreguntaCard = ({ pregunta, esPrimero, esUltimo }: Props) => {
  ...
  const [enunciado, setEnunciado] = useState(
    esFIB ? (pregunta.enunciado_contexto || pregunta.enunciado) : pregunta.enunciado,
  );
  const [puntos, setPuntos] = useState(pregunta.puntos);

  const [opciones, setOpciones] = useState<IOpcionEditor[]>(
    pregunta.opciones.map((op) => ({
      texto: op.texto, es_correcta: op.es_correcta, blank_id: op.blank_id ?? null,
    })),
  );
  ...
  const [items,    setItems]    = useState<IItemFIBEditor[]>(() => esFIB ? getItemsEditor(pregunta) : []);
  const [columnas, setColumnas] = useState<1 | 2 | 3>(() => normalizeColumnas(pregunta.columnas));
  const [dropdownBlancos, setDropdownBlancos] = useState<IDropdownBlancoEditorForm[]>(() => {
    ...
    const ids = [...new Set(pregunta.opciones.map((op) => op.blank_id ?? "blanco1"))];
    return ids.map((bid) => ({
      blank_id: bid,
      opciones: pregunta.opciones.filter((op) => op.blank_id === bid)
  ...
  const handleAbrirEdicion = () => {
    setEnunciado(esFIB ? (pregunta.enunciado_contexto || pregunta.enunciado) : pregunta.enunciado);
    ...
    if (pregunta.tipo === "multiple_dropdowns" && pregunta.opciones.length > 0) {
      const ids = [...new Set(pregunta.opciones.map((op) => op.blank_id ?? "blanco1"))];
      setDropdownBlancos(ids.map((bid) => ({
        blank_id: bid,
        opciones: pregunta.opciones.filter((op) => op.blank_id === bid)
          .map((op) => ({ texto: op.texto, es_correcta: op.es_correcta })),
      })));
  ...
  const handleGuardar = async () => {
    setGuardando(true);
    const payload: Parameters<typeof editarPregunta>[0] = { pregunta_id: pregunta._id, enunciado, puntos };
    ...
    switch (pregunta.tipo) {
      ...
      case "fill_in_multiple_blanks":
        ...
        payload.opciones = items.map((it) => ({
          texto: it.respuesta.trim(), es_correcta: true,
          blank_id: it.id, tipo_pimu: it.tipoPimu,
        }));
        break;
      case "multiple_dropdowns":
        payload.opciones = dropdownBlancos.flatMap((b) =>
          b.opciones.filter((o) => o.texto.trim())
  ...
  return (
    <>
      <Card elevation={0} className="animate-fadeIn"
        sx={{ borderRadius: 3, border: "1px solid #d9e4ee", transition: "box-shadow 0.2s",
        ...
        <CardContent sx={{ p: 0 }}>
          ...
          <div className="flex items-center gap-3 px-5 py-3"
            ...
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#4A6D8C",
          ...
          <div className="px-5 py-4">
            {editando ? (
              <PreguntaEditor
                ...
                items={items}
                columnas={columnas}
                onItemsChange={(nextItems, nextColumnas) => { setItems(nextItems); setColumnas(nextColumnas); }}
              ...
              <PreguntaViewer
                ...
                enunciado_contexto={pregunta.enunciado_contexto || pregunta.enunciado}
