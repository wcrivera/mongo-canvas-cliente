...
interface IItemFIBForm         { id: string; enunciado: string; respuesta: string; tipoPimu: TipoPimu; }
...
type TipoPimu =
  | "numero" | "formula" | "antiderivada" | "conjunto" | "intervalo"
  | "ecuacion" | "punto" | "factorizacion" | "formulaN" | "formulaT"
  | "vector" | "conjunto-vectores";
...
const FormPregunta = ({ quiz_id, onCreada }: Props) => {
  ...
  const agregarItem = () => {
    ...
    setItems(prev => [...prev, { id: `blanco${n}`, enunciado: "", respuesta: "", tipoPimu: "numero" }]);
  };
  const eliminarItem = (idx: number) => {
    if (items.length <= 1) return;
    setItems(prev => prev.filter((_, i) => i !== idx).map((it, i) => ({ ...it, id: `blanco${i + 1}` })));
  };
  ...
  const handleGuardar = async () => {
    ...
    if (tipo === "fill_in_multiple_blanks") {
      if (items.some(it => !it.respuesta.trim())) {
        setError("Todos los ítems deben tener una respuesta."); return;
      }
    }
    ...
    const result = await dispatch(crearPregunta(payload)) as unknown as { ok: boolean; msg?: string };
    setGuardando(false);
    ...
    <div className="flex flex-col gap-4 p-4 bg-white rounded-2xl border border-[#d9e4ee]">
        ...
        <div style={{ background: "#f0f7ff", borderRadius: 10, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div className="flex items-center gap-3">
            ...
            {([1, 2, 3] as (1 | 2 | 3)[]).map(n => (
              <Button key={n} size="small" variant={columnas === n ? "contained" : "outlined"}
                onClick={() => setColumnas(n)}
                sx={{
                  minWidth: 36, height: 28, borderRadius: 1.5, fontSize: 12,
                  ...(columnas === n
          ...
          {items.map((item, idx) => {
            ...
            return (
              <div key={item.id} style={{ background: "white", borderRadius: 8, padding: "10px 12px", border: "1px solid #d9e4ee", display: "flex", flexDirection: "column", gap: 8 }}>
                <div className="flex items-center justify-between gap-2">
                  <Typography variant="caption" sx={{ color: "#2d5be3", fontWeight: 700, fontSize: 12 }}>
                    [{item.id}]
                  </Typography>
                  {items.length > 1 && (
                    <IconButton size="small" onClick={() => eliminarItem(idx)}
                      sx={{ color: "#ef4444" }}>
                ...
                <div className="flex gap-2">
                  <FormControl size="small" sx={{ minWidth: 160 }}>
                    ...
                    <Select value={item.tipoPimu} label="Tipo"
          ...
          <Alert severity="info" sx={{ py: 0.5, fontSize: 12, borderRadius: 2 }}>
            El enunciado general va en el editor de abajo. Cada ítem puede tener su propio enunciado (si lo dejas vacío, se usa el general). Los ítems se muestran en un grid de {columnas} columna{columnas !== 1 ? "s" : ""}.
          </Alert>
        </div>
      )}
      ...
      <div>
        <Typography variant="caption" sx={{ color: "#6793ba", fontWeight: 600, display: "block", mb: 1 }}>
          {esFib ? "Enunciado de contexto (texto introductorio sobre los ítems)" : "Enunciado"}
          {tipo === "multiple_dropdowns" ? " — usa [blanco1], [blanco2], etc. donde corresponda" : ""}
        </Typography>
        <MathTextEditor
      ...
      {(tipo === "multiple_choice" || tipo === "multiple_answers") && (
        <div className="flex flex-col gap-2">
          <Typography variant="caption" sx={{ color: "#6793ba", fontWeight: 600 }}>
            Opciones {esMultiple ? "(selecciona todas las correctas)" : "(selecciona la correcta)"}
          </Typography>
          {opciones.map((op, idx) => (
            <div key={idx} className="flex items-center gap-2">
              <Checkbox checked={op.es_correcta} size="small"
        ...
        <div className="flex flex-col gap-2">
          ...
          {["Verdadero", "Falso"].map((label, idx) => (
            <div key={label} className="flex items-center gap-2">
              <Checkbox checked={opciones[idx]?.es_correcta ?? false} size="small"
          ...
          {respNum.tipo === "exact" && (
            <div className="flex gap-3">
              <TextField label="Valor exacto" type="number" value={respNum.exacto}
                ...
                size="small" sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
              <TextField label="Margen error" type="number" value={respNum.margen}
          ...
          {respNum.tipo === "precision" && (
            <div className="flex gap-3">
              <TextField label="Valor" type="number" value={respNum.exacto}
                onChange={e => setRespNum(r => ({ ...r, exacto: Number(e.target.value) }))}
                size="small" sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
              <TextField label="Decimales" type="number" value={respNum.precision}
      ...
      {tipo === "multiple_dropdowns" && (
        <div className="flex flex-col gap-3">
          ...
          {dropdownBlancos.map((blanco, bidx) => (
            <div key={blanco.blank_id} style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 12px", border: "1px solid #d9e4ee" }}>
              <div className="flex items-center justify-between mb-2">
                <Typography variant="caption" sx={{ color: "#2d5be3", fontWeight: 700 }}>
                  [{blanco.blank_id}]
                </Typography>
              ...
              {blanco.opciones.map((op, oidx) => (
                <div key={oidx} className="flex items-center gap-2 mb-1">
                  <Checkbox checked={op.es_correcta} size="small"
                    onChange={() => setDropdownBlancos(bs => bs.map((b, i) =>
                      i !== bidx ? b : { ...b, opciones: b.opciones.map((o, j) => ({ ...o, es_correcta: j === oidx })) }
                    ))}
                    sx={{ color: "#8daecb", "&.Mui-checked": { color: "#2d5be3" } }} />
                  <TextField value={op.texto} size="small" fullWidth
                    placeholder={`Opción ${oidx + 1}`}
                    onChange={e => setDropdownBlancos(bs => bs.map((b, i) =>
                      i !== bidx ? b : { ...b, opciones: b.opciones.map((o, j) => j === oidx ? { ...o, texto: e.target.value } : o) }
                    ))}
                    sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
                  {blanco.opciones.length > 2 && (
                    <IconButton size="small"
                      onClick={() => setDropdownBlancos(bs => bs.map((b, i) =>
          ...
          <Button size="small" startIcon={<AddIcon />}
            onClick={() => {
              ...
              setDropdownBlancos(bs => [...bs, { blank_id: `blanco${n}`, opciones: [{ texto: "", es_correcta: true }, { texto: "", es_correcta: false }] }]);
            }}
            sx={{ color: "#4A6D8C", alignSelf: "flex-start", textTransform: "none" }}>
            Agregar blanco
          </Button>
