...
import {
  MATH_BLOCK_LABELS,
  MATH_BLOCK_COLORS,
} from "./extensions/MathBlock.extension";
import type { MathBlockType } from "./extensions/MathBlock.extension";
        ...
        <div style={{ position:"absolute",top:"100%",left:0,zIndex:50,background:"white",border:"1px solid #d9e4ee",borderRadius:8,boxShadow:"0 4px 16px rgba(74,109,140,0.15)",minWidth:160,overflow:"hidden" }}>
          {OL_TYPES.map((t) => (
            <button key={t.value} type="button"
              onMouseDown={(e) => {
                e.preventDefault();
                if (!editor.isActive("orderedList")) editor.chain().focus().toggleOrderedList().updateAttributes("orderedList",{listStyleType:t.value}).run();
                else editor.chain().focus().updateAttributes("orderedList",{listStyleType:t.value}).run();
                setOpen(false);
              }}
...
// ── Color picker ──────────────────────────────────────────────────────────────
const COLORS = ["#1f2c38","#4A6D8C","#2d5be3","#1a9e5c","#e74c3c","#e67e22","#9c27b0","#8daecb"];
...
// ── Panel imagen ──────────────────────────────────────────────────────────────
const ImagePanel = ({ editor, isImage, imageAttrs }: { editor: Editor; isImage: boolean; imageAttrs: Record<string,unknown>|null }) => {
  if (!isImage) return null;
  const currentWidth = (imageAttrs?.width as string) ?? "";
  const currentAlign = (imageAttrs?.align as string) ?? "left";
  const applyAttrs = (attrs: Record<string,unknown>) => editor.chain().focus().updateAttributes("image",attrs).run();
  return (
    <div style={{ display:"flex",alignItems:"center",gap:8,padding:"4px 10px",background:"#eef3f8",borderBottom:"1.5px solid #d9e4ee",flexWrap:"wrap" }}>
      <span style={{ fontSize:11,color:"#4A6D8C",fontWeight:600 }}>Imagen:</span>
      <div style={{ display:"flex",alignItems:"center",gap:4 }}>
        <span style={{ fontSize:11,color:"#6793ba" }}>Ancho:</span>
        <input type="text" defaultValue={currentWidth} placeholder="ej: 300px"
          onBlur={(e) => applyAttrs({ width: e.target.value.trim()||null })}
          onKeyDown={(e) => { if(e.key==="Enter"){ applyAttrs({width:(e.target as HTMLInputElement).value.trim()||null});(e.target as HTMLInputElement).blur();} }}
      ...
      <div style={{ display:"flex",alignItems:"center",gap:4 }}>
        ...
        {(["left","center","right"] as const).map((align) => (
          <button key={align} type="button" onMouseDown={(e) => { e.preventDefault(); applyAttrs({align}); }}
            style={{ width:26,height:26,borderRadius:4,border:"1px solid",borderColor:currentAlign===align?"#4A6D8C":"#c9dae8",background:currentAlign===align?"#4A6D8C":"white",color:currentAlign===align?"white":"#6793ba",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center" }}>
...
export function Toolbar({ editor, onInsertMath, fullscreen, onToggleFullscreen, showHtml, onToggleHtml }: ToolbarProps) {
  const state = useEditorState({
    ...
    selector: (ctx: { editor: Editor | null }) => ({
      ...
      imageAttrs:   ctx.editor?.isActive("image") ? ctx.editor.getAttributes("image") : null,
    }),
  });
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ display:"flex",flexWrap:"wrap",alignItems:"center",gap:2,padding:"6px 10px",background:"#f7f9fc",borderBottom:"1.5px solid #d9e4ee",minHeight:46,position:"sticky",top:0,zIndex:10 }}>
        ...
        {/* Formato */}
        <div style={{ display:"flex",gap:1 }}>
          <ToolbarBtn title="Negrita"    active={state?.isBold}      onClick={md(()=>editor.chain().focus().toggleBold().run())}><FormatBoldIcon fontSize="small"/></ToolbarBtn>
          <ToolbarBtn title="Itálica"    active={state?.isItalic}    onClick={md(()=>editor.chain().focus().toggleItalic().run())}><FormatItalicIcon fontSize="small"/></ToolbarBtn>
          <ToolbarBtn title="Subrayado"  active={state?.isUnderline} onClick={md(()=>editor.chain().focus().toggleUnderline().run())}><FormatUnderlinedIcon fontSize="small"/></ToolbarBtn>
        ...
        {/* Alineación */}
        <div style={{ display:"flex",gap:1 }}>
          <ToolbarBtn title="Izquierda" active={state?.isAlignL} onClick={md(()=>editor.chain().focus().setTextAlign("left").run())}><FormatAlignLeftIcon fontSize="small"/></ToolbarBtn>
          ...
          <ToolbarBtn title="Derecha"   active={state?.isAlignR} onClick={md(()=>editor.chain().focus().setTextAlign("right").run())}><FormatAlignRightIcon fontSize="small"/></ToolbarBtn>
        </div>
        <Sep/>
        ...
        <div style={{ display:"flex",gap:1,alignItems:"center" }}>
          ...
          <ToolbarBtn title="Código" active={state?.isCode} onClick={md(()=>editor.chain().focus().toggleCodeBlock().run())}><CodeIcon fontSize="small"/></ToolbarBtn>
        </div>
        <Sep/>
        ...
        <div style={{ display:"flex",gap:1 }}>
          ...
          <ToolbarBtn title="Insertar imagen" onClick={(e)=>{ e.preventDefault(); const url=window.prompt("URL de la imagen:"); if(url)editor.chain().focus().setImage({src:url}).run(); }}><ImageIcon fontSize="small"/></ToolbarBtn>
        </div>
        <Sep/>
        ...
        {onToggleHtml && (
          <ToolbarBtn title={showHtml?"Ocultar HTML":"Ver HTML"} active={showHtml} onClick={(e)=>{ e.preventDefault(); onToggleHtml(); }}>
            <CodeOffIcon fontSize="small"/>
          </ToolbarBtn>
        )}
        {onToggleFullscreen && (
          <ToolbarBtn title={fullscreen?"Salir":"Pantalla completa"} onClick={(e)=>{ e.preventDefault(); onToggleFullscreen(); }}>
            {fullscreen?<FullscreenExitIcon fontSize="small"/>:<FullscreenIcon fontSize="small"/>}
          </ToolbarBtn>
        )}
      </div>

      {/* ══ Barra de componentes matemáticos ══ */}
      <div style={{ display:"flex",flexWrap:"wrap",alignItems:"center",gap:4,padding:"5px 10px",background:"#f0f4f8",borderBottom:"1.5px solid #d9e4ee",minHeight:36 }}>

        {/* Componentes matemáticos */}
        {(Object.keys(MATH_BLOCK_LABELS) as MathBlockType[]).map((tipo) => {
          const colors = MATH_BLOCK_COLORS[tipo];
          return (
            <button key={tipo} type="button" title={`Insertar ${MATH_BLOCK_LABELS[tipo]}`}
              onMouseDown={(e) => {
                e.preventDefault();
                editor.chain().focus().insertMathBlock(tipo).run();
              }}
              style={{ display:"inline-flex",alignItems:"center",padding:"0 9px",height:26,borderRadius:5,border:`1.5px solid ${colors.border}50`,background:colors.bg,color:colors.title,cursor:"pointer",fontSize:11,fontWeight:600,flexShrink:0,whiteSpace:"nowrap" }}>
              {MATH_BLOCK_LABELS[tipo]}
            </button>
          );
        })}
      </div>
      ...
      {/* Panel imagen */}
      <ImagePanel editor={editor} isImage={state?.isImage ?? false} imageAttrs={state?.imageAttrs ?? null}/>
    </div>
  );
}