  ...
  return async (dispatch: AppDispatch) => {
    ...
    try {
      const resp = await fetchConToken(
        `api/admin/quizzes/capitulo/${capitulo_id}`,
      );
...
export const obtenerEjercicios = ({ capitulo_id }: { capitulo_id: string }) => {
  return async (dispatch: AppDispatch) => {
    ...
    try {
      const resp = await fetchConToken(
        `api/admin/quizzes/ejercicios/${capitulo_id}`,
      );
      const body = await resp.json();
      if (body.ok) {
        dispatch(setQuizzes(body.data));
...
export const cambiarPositionEjercicioQuiz =
  ({ quiz_id, direction }: { quiz_id: string; direction: "up" | "down" }) =>
  async (dispatch: AppDispatch) => {
    const resp = await fetchConToken(
      `api/admin/quizzes/${quiz_id}/position`,
      { direction },
      "PATCH",
    );
    const body = await resp.json();
...
export const eliminarQuiz = ({ quiz_id }: { quiz_id: string }) => {
  return async (dispatch: AppDispatch) => {
    ...
    try {
      ...
      const body = await resp.json();
      if (body.ok) {
        dispatch(eliminarQuizState(quiz_id));
        dispatch(endLoadingQuiz());
        return { ok: true };
      }
      dispatch(setErrorQuiz(body.msg));
...
export const cambiarPositionPregunta = ({
  ...
  return async (dispatch: AppDispatch) => {
    ...
    try {
      const resp = await fetchConToken(
        `api/admin/quizzes/preguntas/${pregunta_id}/position`,
        { direction },
        "PATCH",
      );
      const body = await resp.json();
...
// ─── Editar ítem individual de pregunta FIB ───────────────────────────────────
