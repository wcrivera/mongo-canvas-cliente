...
const initialState: AuthState = {
  ...
  perfilCargado:      !tokenGuardado,  // si no hay token, no hay nada que cargar
  isLoading:          false,
  error:              null,
};
...
export const authSlice = createSlice({
  ...
  reducers: {
    setCredenciales: (state, action: PayloadAction<{
      ...
      state.perfilCargado = false;  // recién logueado, perfil aún no cargado
    },
    setPerfil: (state, action: PayloadAction<{
      nombre: string;
      tiene_token_canvas: boolean;
    }>) => {
      state.nombre             = action.payload.nombre;
      state.tiene_token_canvas = action.payload.tiene_token_canvas;
      state.perfilCargado      = true;  // ← perfil ya cargado
    },
    ...
    setErrorAuth: (state, action: PayloadAction<string>) => {
      state.isLoading     = false;
      state.perfilCargado = true;  // aunque falle, no bloqueamos la app
